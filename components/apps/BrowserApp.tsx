...
const getInitialState = (): { tabs: Tab[], activeTabId: string | null } => {
    try {
        const savedState = localStorage.getItem('browser_tabs_state');
        if (savedState) {
            const parsed = JSON.parse(savedState);
            if (Array.isArray(parsed.tabs) && parsed.tabs.length > 0 && (typeof parsed.activeTabId === 'string' || parsed.activeTabId === null)) {
                // Ensure activeTabId is valid, otherwise default to first tab
                const activeTabExists = parsed.tabs.some((t: Tab) => t.id === parsed.activeTabId);
                return {
                    tabs: parsed.tabs,
                    activeTabId: activeTabExists ? parsed.activeTabId : parsed.tabs[0]?.id || null,
                };
            }
        }
    } catch (e) {
        console.error("Failed to load browser state from localStorage:", e);
    }
...
const BrowserApp: React.FC = () => {
    ...
    useEffect(() => {
        try {
            const stateToSave = JSON.stringify({ tabs, activeTabId });
            localStorage.setItem('browser_tabs_state', stateToSave);
        } catch (e) {
            console.error("Failed to save browser state to localStorage", e);
        }
    }, [tabs, activeTabId]);
